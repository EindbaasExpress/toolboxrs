...
pub fn process_b64(value: &str, operation_type: Base64OperationType) -> String {
    ...
    let output: String = match operation_type {
        Base64OperationType::DecodeURLSafe => {
            match general_purpose::URL_SAFE_NO_PAD.decode(removing_padding(value)) {
                Ok(vec) => String::from_utf8(vec).unwrap(),
                Err(error) => panic!("this error was thrown: {}", error),
            }
        }
        Base64OperationType::EncodeURLSafe => {
        ...
        Base64OperationType::EncodeStandard => {
...
mod tests {
    ...
    fn test_base64encode_decode() {
        ...
        assert_eq!(
            orig,
            process_b64(&encoded, Base64OperationType::DecodeStandard)
        );
        ...
        assert_eq!(
            orig,
            process_b64(&encoded2, Base64OperationType::DecodeURLSafe)
        );
