use clap::Parser;
use cli::{Base64Commands, CidrCommands, Cli, Commands};
use shared::{hash_once_out_loud, ipv4_to_cidr};
    ...
    match &cli.command {
        Commands::Cidr(args) => match &args.cidr_commands {
            CidrCommands::Ipaddress(args) => {
                ...
                let cidr_range_output = ipv4_to_cidr(&args.ip_address);
                println!("{}", cidr_range_output);
            }
            CidrCommands::Subnetmask(args) => {
                println!(
                    "This feature is not implemented yet; you passed {} as mask",
                    args.subnetmask
                );
            }
        },
        Commands::Base64(args) => match &args.base64_commands {
            Base64Commands::Encode(args) => {
                println!(
                    "You passed a value to be base64-encoded, the url_safe option is set to {}",
                    args.url_safe
                );
                if args.url_safe {
            ...
            Base64Commands::Decode(args) => {
                println!(
                    "You passed a value to be base64-decoded, the url_safe option is set to {}",
                    args.url_safe
                );
                if args.url_safe {
        ...
        Commands::Server(args) => {
            println!(
                "localhost:{} is used as the port to run the server \n",
                args.port
            );
            match args.port.parse::<u16>() {
                Ok(port) => {
                    ...
                    let asset_folder =
                        warp::path("assets").and(warp::fs::dir("./wasm/toolboxrs-ui/dist/assets"));
